...
- Gestión de categorías: acciones `addCategory`/`updateCategory`/`deleteCategory` (mismo archivo), UI en la sección "Categorías" de `app/(dashboard)/dashboard/lists/[listId]/page.tsx`, componente `components/list-categories.tsx`. Nombre único por lista (`@@unique([listId, name])`); no hay protección contra borrar la última categoría de una lista (es un estado válido, no una lista rota).
- **Color por categoría**: `Category.color` es texto libre (no un enum de la base), validado contra una paleta curada (`CATEGORY_COLORS` en `lib/categories.ts` — pares `bg-*-100/text-*-700` de Tailwind ya armonizados, no un color picker de RGB libre, para no tener que calcular contraste en runtime). `categoryBadgeClasses()`/`categorySwatchClasses()` resuelven el color a clases, con fallback a `DEFAULT_CATEGORY_COLOR` si el valor guardado no está en la paleta actual (la columna es texto libre a propósito, para poder ampliar la paleta sin migración). El picker es un swatch de círculos clicables (`ColorSwatches` en `components/list-categories.tsx`), no un `<input type="color">`.
