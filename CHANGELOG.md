...
- Job `docker` en `ci.yml` (`needs: [verify, test]`): construye la imagen para `linux/amd64` y `linux/arm64` (Raspberry Pi 4B, vía QEMU) y la publica en `ghcr.io/zaphold2k/giftlist` con tags por branch/PR/sha en cada push a una rama; en PRs solo construye `amd64`, sin publicar. Incluye smoke test post-build (`docker run` + `curl` a `/api/health`) sobre la imagen `amd64` y una base de datos vacía.
